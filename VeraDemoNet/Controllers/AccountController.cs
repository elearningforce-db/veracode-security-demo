...
{  
    ...
    public class AccountController : AuthControllerBase
    {
        ...
        public ActionResult GetLogin(string ReturnUrl = "")
        {
            ...
            /* START BAD CODE */
            var deserializedUser = JsonConvert.DeserializeObject<CustomSerializeModel>(Encoding.UTF8.GetString(unencodedUserDetails));
            /* END BAD CODE */
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    using (EventLog eventLog = new EventLog("Application"))
                    {
                        ...
                        eventLog.WriteEntry("Entering PostLogin with username " + loginViewModel.UserName, EventLogEntryType.Information, 101, 1);
                    }
                    ...
                    if (loginViewModel.RememberLogin)
                    {
                        ...
                        var faCookie =
                            new HttpCookie(COOKIE_NAME, Convert.ToBase64String(Encoding.UTF8.GetBytes(json)))
                            {
        ...
        public ActionResult PostRegisterFinish(RegisterViewModel model)
        {
            if (model.Password != model.CPassword)
            {
                ...
                model.Error = "The Password and Confirm Password values do not match. Please try again.";
                return View(model);
            }
